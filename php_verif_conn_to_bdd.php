...
// on pourrait aussi marquer ça : $bdd = new PDO('mysql:host=localhost;dbname=espacemembre', 'root', 'root'); cela revient au meme :) 
...
if(isset($_POST['formconnexion'])) // si on a cliqué sur le bouton connexion
{
   ...
   if(!empty($mailconnect) AND !empty($mdpconnect)) //si on a bien rempli les champs mais de toutes façons avec le required dans le html ça nous oblige de tous les remplir mais mieux vaut 2 fois qu'une! 
   {
      ...
      $userexist = $requser->rowCount();// on crée une nouvelle variable pour compter le nombre de colonne pour que le mail et le mot de passe correspondent. 

      if($userexist == 1) 
      {
     
         header("Location: http://localhost:8888/code%20actuel/pagecapteursdivercity.php");//on le redirige vers le profil de la personne ! 
      } 
      else 
      {
